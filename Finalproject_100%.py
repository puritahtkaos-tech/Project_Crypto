...
# ==========================================
try:
    API_KEY = st.secrets["GEMINI_API_KEY"] # หรือใส่ "API_KEY_ของคุณ" ถ้ารันในคอม
...
st.markdown("""
    ...
    .main-title { text-align: center; color: #ffffff; margin-bottom: 20px; }
...
# ==========================================
# 5. หน้า Login / Register (เพิ่ม Social Login UI)
# ==========================================
def show_auth_page():
    # ใส่รูปแบนเนอร์ด้านบน
    st.image("https://images.unsplash.com/photo-1621416894569-0f39ed31d247?q=80&w=1600&auto=format&fit=crop", use_container_width=True)
        ...
        if st.session_state.auth_mode == 'login':
            ...
            if st.button("เข้าสู่ระบบ", use_container_width=True, type="primary"):
                ...
                if success:
                    ...
                    # อัปเดตโครงสร้างกระเป๋าเงินเก่า ให้รองรับเหรียญใหม่ (กันพัง)
            ...
            # เพิ่มปุ่ม Social Login
            st.markdown("<hr><p style='text-align: center; color: #888;'>หรือเข้าสู่ระบบด้วย</p>", unsafe_allow_html=True)
            c1, c2, c3, c4 = st.columns(4)
            with c1: st.button("🔵 FB", use_container_width=True, help="รอการเชื่อมต่อ API")
            with c2: st.button("🟢 Line", use_container_width=True, help="รอการเชื่อมต่อ API")
            with c3: st.button("👾 Discord", use_container_width=True, help="รอการเชื่อมต่อ API")
            with c4: st.button("🔴 Google", use_container_width=True, help="รอการเชื่อมต่อ API")
...
if 'raw_data' in st.session_state and len(st.session_state.raw_data) > 0:
    ...
    # --- TAB 2: พอร์ตโฟลิโอส่วนตัว (เพิ่มฟีเจอร์ใหม่) ---
    with tab2:
        st.subheader("💼 สรุปกระเป๋าเงินของคุณ")
        total_crypto_value = 0
        portfolio_data = []
        
        # คำนวณมูลค่ารวมคร่าวๆ (อิงจากราคาล่าสุดของเหรียญที่เลือกหน้าจอ)
        for coin_name, ticker in coin_dict.items():
            amount = st.session_state.crypto_wallet.get(ticker, 0)
            if amount > 0:
                # เพื่อความรวดเร็ว ระบบจะใช้ราคาล่าสุดของเหรียญที่เปิดดูอยู่ในการคำนวณแทน
        ...
        if portfolio_data:
            ...
            st.info("ยังไม่มีเหรียญในกระเป๋า เริ่มต้นเทรดกันเลย!")
    ...
    with tab3:
        ...
        if st.button("✨ กดเพื่อวิเคราะห์แนวโน้ม"):
            if ai_ready:
                prices = [d['price'] for d in st.session_state.raw_data[-14:]] # ส่งแค่ 14 วันล่าสุดไปให้ AI ไม่ให้โควต้าเต็ม
        ...
        st.markdown("---")
        st.subheader("📉 ค้นหาด้วย Algorithm")
        col_sort, col_search = st.columns(2)
        with col_sort:
            if st.button("1. จัดเรียงราคา (Quick Sort)"):
                random_quick_sort(st.session_state.sorted_data, 0, len(st.session_state.sorted_data)-1)
                st.session_state.is_sorted = True
                st.success("จัดเรียงเสร็จสิ้น!")
        with col_search:
            target_input = st.text_input("2. ป้อนราคาเป้าหมาย (Jump Search):")
            if st.button("🔍 ค้นหา"):
                if not st.session_state.get('is_sorted', False): st.error("⚠️ ต้องกดปุ่มจัดเรียงราคาก่อน!")
                elif not target_input.replace(",", "").isdigit(): st.warning("⚠️ ใส่ตัวเลขเท่านั้น")
                else:
                    target = int(target_input.replace(",", ""))
                    res = jump_search(st.session_state.sorted_data, target)
                    if res: st.success(f"🎯 เจอแล้ว! ราคา {target:,} บาท วันที่ {res['date']}")
                    else: st.error(f"❌ ไม่พบราคา {target:,} บาท ในช่วงเวลานี้")
    ...
    # หน้าจอว่างก่อนกดดึงข้อมูล ให้โชว์รูปใหญ่ๆ รอก่อนเลย
    st.image("https://images.unsplash.com/photo-1518546305927-5a555bb70208?q=80&w=2000&auto=format&fit=crop", use_container_width=True)
